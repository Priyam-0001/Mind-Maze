...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, teamName, onLogout }) => {
  return (
    <div className="w-64 flex flex-col h-screen border-r border-zinc-800 bg-black sticky top-0">
      <div className="p-6">
        <h1 className="text-2xl pixel-font text-orange-500 orange-glow mb-2">MINDMAZE</h1>
        <p className="text-zinc-500 text-sm font-bold tracking-widest">VERSION 3.0</p>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 space-y-2">
        ...
        >
          <ICONS.Puzzle />
          <span className="font-semibold uppercase text-xs tracking-wider">Quests</span>
        </button>
        <button
        ...
        >
          <ICONS.Trophy />
          <span className="font-semibold uppercase text-xs tracking-wider">Leaderboard</span>
        </button>
      </nav>

      <div className="p-4 mt-auto">
        <div className="bg-zinc-900 rounded-xl p-4 mb-4 border border-zinc-800">
          <p className="text-zinc-500 text-[10px] font-bold uppercase mb-1">Signed in as</p>
          <p className="text-zinc-200 font-bold truncate">{teamName}</p>
        </div>
        <button
          onClick={onLogout}
